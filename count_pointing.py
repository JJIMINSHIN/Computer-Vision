...
ret, fg = cv2.threshold(dst, 0.5*dst_max(), 255, 0)
cv2.imshow('fg', fg)
_, bg = cv2.threshold(dst, o.3*dst_max(), 255, cv2_THRESHOLD_BINARY_INV)
bg_dst = cv2.distanceTransform(bg, cv2.DIST_L2, 5)
bg_dst = ((dst/(dst.max()-dst.min()*255)).asType(np.uint8)
_, bg = cv2.threshold(bg_dst,bg_dst.max(), 255, cv2.THRESH_BINARY)

_, inv_bg = cv2.threshold(bg)
